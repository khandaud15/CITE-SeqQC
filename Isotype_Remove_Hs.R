...
write.table(output_rank, file = paste(arguments$o, "_FinalgeneADT-Rank.txt", sep = ""),  row.names = F, col.names = T, sep ="\t", quote = F)
...
fwrite(output, file = paste(arguments$o, "_ADT_clean.txt", sep = ""),  row.names = F, col.names = T, sep ="\t", quote = F)
sessionInfo()
quit()
